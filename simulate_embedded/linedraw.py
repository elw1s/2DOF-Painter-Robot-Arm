...
# file settings
export_path = "/tmp/cse396/out.svg"
svg_folder = "/tmp/cse396/"
json_folder = "/tmp/cse396/"
...
def vectorise(
    ...
    possible = [
        image_filename,
        "/tmp/cse396/" + image_filename,
        "/tmp/cse396/" + image_filename + ".jpg",
        "/tmp/cse396/" + image_filename + ".jpeg",
        "/tmp/cse396/" + image_filename + ".png",
        "/tmp/cse396/" + image_filename + ".tif",
    ]
...
def resize_image(image, resolution, divider=1):
    print("Values of the first row in resizeImage (before): ", end="")
    for x in range(image.size[0]):
        pixel_value = image.getpixel((x, 0))
        print(pixel_value, end=" ")
    print()
    print("Expected:",(int(resolution/divider)), "," , (int(resolution / divider * image.size[1] / image.size[0])))
    print("resolution:",resolution," - divider:",divider)
    ret = image.resize(
        (
    ...
    print("Values of the first row in resizeImage (after): ", end="")
    for x in range(ret.size[0]):
        pixel_value = ret.getpixel((x, 0))
        print(pixel_value, end=" ")
    print()
    return ret
...
def getcontours(image, draw_contours=2):
    ...
    print(f"First pixel RGB values: {first_pixel}")
    #image.save("/home/arda/Desktop/CSE396/BrachioGraph/images/imageBeforeFindEdgesPY.jpg")
    image = find_edges(image)

    threshold = image.getpixel((0, 0))

    # Print the RGB values of the first pixel
    print(f"Threshold RGB values: {threshold}")

    IM1 = image.copy()
    IM2 = image.rotate(-90, expand=True).transpose(Image.FLIP_LEFT_RIGHT)

    #image.save("/home/arda/Desktop/CSE396/BrachioGraph/images/imageAfterFindEdgesPY.jpg")
    #IM1.save("/home/arda/Desktop/CSE396/BrachioGraph/images/IM1PY.jpg")
    #IM2.save("/home/arda/Desktop/CSE396/BrachioGraph/images/IM2PY.jpg")

    dots1 = getdots(IM1)
    image_filename = "cat"
    #f = open(svg_folder + image_filename + "dots1PY.svg", "w")
    #f.write(makesvg(dots1))
    #f.close()
    #print("dots1: ", dots1)
    contours1 = connectdots(dots1)
    #f = open(svg_folder + image_filename + "contours1PY.svg", "w")
    #f.write(makesvg(contours1))
    #f.close()
    dots2 = getdots(IM2)
...
def find_edges(image):
    ...
    if no_cv:
        ...
        #Image.fromarray(im).save("/home/arda/Desktop/CSE396/BrachioGraph/images/blurredPY.jpg")
        im = cv2.Canny(im, 100, 200)
        #Image.fromarray(im).save("/home/arda/Desktop/CSE396/BrachioGraph/images/edgesPY.jpg")
        image = Image.fromarray(im)
        #Image.fromarray(im).point(lambda p: p > 128 and 255).save("/home/arda/Desktop/CSE396/BrachioGraph/images/thresholdPY.jpg")
...
def getdots(IM):
    ...
    # Access the first row of pixel values in PX
    # first_row_PX = [PX[x, 0] for x in range(w)]
    # print("First row pixel values in PX:", first_row_PX)

    # # Access the first pixel value of IM
    # first_pixel_IM = IM.getpixel((0, 0))
    # print(f"First pixel value in IM: {first_pixel_IM}")
    # print("IM.size:", w , "," , h)
...
fileName = "sent"
image_to_json(fileName,1024,2,1,16,1)
